...
void CAnd1Alg::SetInputCfg(int index, TStreamCfg *cfg)
{
	CTypeProp tp;
...
int CAnd1Alg::GetOutputCfg(int index, TStreamCfg *cfg)
{
	CTypeProp tp;
...
void CAnd1Alg::ResetState(void)
{
	CImageAlgBase::ResetState();
}

int CAnd1Alg::Write(int index, char *buf, int len)
{
	ASSERT((index == 0) || (index == 1));
	return this->m_linebuf[index].Write(buf, len);
}

int CAnd1Alg::Read(int index, char *buf, int len)
{
	int i, val1, val2;
