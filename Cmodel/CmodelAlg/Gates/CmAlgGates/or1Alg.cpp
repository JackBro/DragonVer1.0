...
void COr1Alg::SetInputCfg(int index, TStreamCfg *cfg)
{
	CTypeProp tp;
...
int COr1Alg::GetOutputCfg(int index, TStreamCfg *cfg)
{
	CTypeProp tp;
...
void COr1Alg::ResetState(void)
{
	CImageAlgBase::ResetState();
}

int COr1Alg::Write(int index, char *buf, int len)
{
	ASSERT((index == 0) || (index == 1));
	return this->m_linebuf[index].Write(buf, len);
}

int COr1Alg::Read(int index, char *buf, int len)
{
	int i, val1, val2;
