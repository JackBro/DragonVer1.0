...
void CSub1Alg::SetInputCfg(int index, TStreamCfg *cfg)
{
	CTypeProp tp;
...
int CSub1Alg::GetOutputCfg(int index, TStreamCfg *cfg)
{
	CTypeProp tp;
...
void CSub1Alg::ResetState(void)
{
	CImageAlgBase::ResetState();
}

int CSub1Alg::Write(int index, char *buf, int len)
{
	ASSERT((index == 0) || (index == 1));
	return this->m_linebuf[index].Write(buf, len);
}

int CSub1Alg::Read(int index, char *buf, int len)
{
	int i, val1, val2;
