		...
		while(<IN>) 
		{
			...
			elsif (/Name\=\"VCResourceCompilerTool\"/)
			{
				...
				if($IsDebug ==1)
				{
					...
					Culture\=\"1033\"
					AdditionalIncludeDirectories\=\"\$(SolutionDir)Vmainfrm\\res\"
					ShowProgress\=\"FALSE\"
					ResourceOutputFileName\=\"\$(SolutionDir)Vmainfrm\\res\\\$(InputName).res\"\/>
					",
					"Name",OUT);
			...
			elsif (/<FileConfiguration/)
			{
				while(<IN>)
				{
					last if (/<\/FileConfiguration>/);
				}
			}
			else
			{
