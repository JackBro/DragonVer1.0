...
#define MOBITAG (ULONG) 'KluB'

#define MOBI_DIRECT_IO

#undef ExAllocatePool
#define ExAllocatePool(type, size) \
    ExAllocatePoolWithTag(type, size, MOBITAG);



#ifdef DBG

#define MobiUsb_DbgPrint(level, _x_) \
            if((level) <= DebugLevel) { \
                DbgPrint _x_; \
            }
...
typedef struct _GLOBALS {
    ...
    UNICODE_STRING MobiUsb_RegistryPath;
...
#define MOBIUSB_PACK_READ_TIMEOUT   1
#define MOBIUSB_PACK_WRITE_TIMEOUT   100


#define MOBIUSB_MAX_TRANSFER_SIZE   4096
#define MOBIUSB_TEST_BOARD_TRANSFER_BUFFER_SIZE (64 *1024 )
...
#define MOBIUSB_REGISTRY_PARAMETERS_PATH  \
	L"\\REGISTRY\\Machine\\System\\CurrentControlSet\\SERVICES\\MOBIUSB\\Parameters"


typedef struct _MOBIUSB_PIPE_CONTEXT {

    BOOLEAN PipeOpen;

} MOBIUSB_PIPE_CONTEXT, *PMOBIUSB_PIPE_CONTEXT;

typedef struct _MOBI_BULK_CONTEXT {

    LIST_ENTRY ReadQueue;

} MOBI_BULK_CONTEXT, *PMOBI_BULK_CONTEXT;
...
typedef struct _DEVICE_EXTENSION {
    ...
    // Pipe context for the mobiusb driver
