/***********************************************************************************/
/** 	OSTime.c 	                                                              **/
/**     Function:   OS time operation functions define                            **/
/** 	Author: 	zhangjunqi                                                    **/
/**	    Date:		2005-05-25                                                    **/
...
UINT8  OSTimeDlyHMSM (UINT8 hours, UINT8 minutes, UINT8 seconds, UINT16 milli)
{
    ...
    if (hours > 0 || minutes > 0 || seconds > 0 || milli > 0) {
        ...
        ticks = ((UINT32)hours * 3600L + (UINT32)minutes * 60L + (UINT32)seconds) * OS_TICKS_PER_SEC
              + OS_TICKS_PER_SEC * ((UINT32)milli + 500L / OS_TICKS_PER_SEC) / 1000L;
        loops = (UINT16)(ticks >> 16);           /* Compute the integral number of 65536 tick delays   */
        ticks &= 0xffff;                         /* Obtain  the fractional number of ticks             */
...
UINT8  OSTimeDlyResume(UINT8 prio)
{
    OS_TCB    *ptcb;
