...
//extern void __RdPage528(UINT8	*);	/*these two function are writed in ASM*/
//extern void __WrPage528(UINT8	*);
UINT8	Verify_Buf[528];
...
/********************************************************/
/*
int VerifyPage(unsigned int block,unsigned int page,UINT8	*buffer)
{
...
int WritePage(unsigned int block,unsigned int page,unsigned char *pPage)  
{
	unsigned int blockPage = (block<<5) + page;
	int i;
	UINT8	*Source;                                      //the begin addr of data to write
	Source=pPage;
	FC_CMD;
	rKM29UXDATA = 0x80;
	FC_ADDR;
